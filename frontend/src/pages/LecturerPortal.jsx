...
const LecturerPortal = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f5] text-[#344767] p-6 relative overflow-hidden">
      ...
      <div className="absolute top-[-30%] right-[-10%] w-[70%] h-[70%] rounded-full bg-[#344767]/3 blur-[150px] pointer-events-none"></div>

      {/* Nav */}
      <div className="max-w-6xl mx-auto flex justify-between items-center mb-6 pb-6 border-b border-gray-200 relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-[#344767] flex items-center gap-2">
              {displayDeptName}
            </h1>
            <p className="text-xs text-[#8392ab]">Lecturer Portal • Review and sign documents</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <NotificationPanel />
          <span className="text-[#8392ab] text-sm hidden md:inline">Logged in as: <span className="font-semibold text-[#344767]">{displayUsername}</span></span>
          <button
            onClick={handleLogout}
            className="bg-gray-200 hover:bg-gray-100 text-[#344767] font-semibold px-4 py-2 rounded-lg border border-gray-300 transition-all text-sm"
          >
            Sign Out
          </button>
        </div>
      ...
      <div className="max-w-6xl mx-auto mb-6 flex gap-4 relative z-10">
        <button
          ...
          className={`px-5 py-2.5 rounded-lg font-bold text-sm transition ${activeTab === 'sessions' ? 'bg-gradient-to-br from-[#14172B] to-[#3A416F] text-white shadow-lg shadow-[#344767]/20' : 'bg-white text-[#8392ab] hover:bg-gray-100 border border-gray-200'}`}
        >
          Daily Sessions
        </button>
        <button
          onClick={() => setActiveTab('reports')}
          className={`px-5 py-2.5 rounded-lg font-bold text-sm transition ${activeTab === 'reports' ? 'bg-gradient-to-br from-[#14172B] to-[#3A416F] text-white shadow-lg shadow-[#344767]/20' : 'bg-white text-[#8392ab] hover:bg-gray-100 border border-gray-200'}`}
        >
      ...
      <div className="max-w-6xl mx-auto mb-6 bg-white border border-gray-200 p-4 rounded-xl relative z-10">
        <h3 className="text-xs font-semibold text-[#8392ab] uppercase tracking-wider mb-3 flex items-center gap-1.5">
        ...
        {loadingClasses ? (
          <div className="flex gap-2">
            <div className="h-10 w-24 bg-[gray-200]/40 animate-pulse rounded-lg"></div>
            <div className="h-10 w-32 bg-[gray-200]/40 animate-pulse rounded-lg"></div>
          </div>
        ) : assignedClasses.length === 0 ? (
          <p className="text-xs text-[#8392ab]">No courses or classes currently assigned. Please contact the Administrator to upload your class distribution.</p>
        ) : (
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={`px-3 py-2 rounded-lg text-xs font-semibold border transition-all ${
                !selectedClass
                  ...
                  : 'bg-gray-50 border-gray-200 text-[#8392ab] hover:border-[#344767]'
              }`}
            >
              All Classes
            </button>
            {assignedClasses.map((ac) => (
              ...
              >
                ...
                <span className={`text-[10px] mt-0.5 font-medium ${selectedClass?.assignmentId === ac.assignmentId ? 'text-indigo-200' : 'text-[#8392ab]'}`}>
                  {ac.classDisplayName}
                </span>
              </button>
            ))}
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white backdrop-blur-xl border border-gray-200 p-6 rounded-2xl shadow-xl min-h-[300px]">
            <h2 className="text-lg font-bold text-[#344767] mb-4">
            ...
            {loading ? (
              ...
              <div className="text-[#8392ab] text-sm text-center py-10">
              ...
              <div className="space-y-3">
                  ...
                  <div
                    ...
                    className={`p-4 border rounded-xl transition-all cursor-pointer text-left ${
                      selectedSession?.id === s.id
                        ? 'bg-gray-50 border-[#344767] shadow-md shadow-[#344767]/5'
                        : 'bg-gray-50 border-gray-200 hover:border-[#344767]'
                    }`}
                  >
                    <p className="font-bold text-[#344767] text-sm">{s.courseName}</p>
                    <p className="font-mono text-xs text-[#8392ab] mt-1">{s.courseCode}</p>
                    {s.classDisplayName && (
                      <p className="text-xs text-indigo-400 mt-1 font-semibold">{s.classDisplayName}</p>
                    )}
                    <p className="text-xs text-[#8392ab] mt-3 flex items-center gap-1">
                ...
                {activeTab === 'reports' && getFilteredReports().map((r) => (
                  ...
                  >
                    <p className="font-bold text-[#344767] text-sm">{r.course.name}</p>
                    <p className="text-xs font-semibold text-indigo-400 mt-1">Class: {r.class.displayName}</p>
                    <p className="text-[10px] text-[#8392ab] mt-2 flex items-center gap-1">
        ...
        <div className="lg:col-span-2 space-y-6">
          {!selectedSession && !selectedReport ? (
            <div className="bg-white backdrop-blur-xl border border-gray-200 p-8 rounded-2xl shadow-xl flex flex-col items-center justify-center text-center min-h-[450px]">
              <div className="p-4 rounded-full bg-gray-50 border border-gray-200 text-[#8392ab] mb-4">
                <svg className="w-12 h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
              </div>
              <h3 className="text-lg font-bold text-[#344767] mb-2">No Item Selected</h3>
              <p className="text-sm text-[#8392ab] max-w-sm">Select a pending item from the left list to review and sign off.</p>
            </div>
          ) : (
            <div className="bg-white backdrop-blur-xl border border-gray-200 p-6 rounded-2xl shadow-xl space-y-6">
                ...
                <>
                  {loadingDetails ? (
                    ...
                    <div className="space-y-6">
                      <div>
                        <h4 className="text-sm font-semibold text-[#344767] mb-3">Attendance Report Summary</h4>
                      ...
                      <div className="flex gap-4">
                        <button type="button" onClick={() => setSelectedSession(null)} className="flex-1 py-3 bg-gray-200 hover:bg-gray-100 text-[#344767] font-semibold rounded-xl transition-all text-sm">
                          Close Review
                        </button>
                        <button onClick={handleApproveSession} disabled={approving || !signature} className="flex-1 bg-gradient-to-br from-[#14172B] to-[#3A416F] hover:opacity-90 disabled:bg-gray-50 disabled:text-[#8392ab] text-white font-bold py-3 px-4 rounded-xl shadow-lg transition-all text-sm flex items-center justify-center gap-2">
              ...
              {activeTab === 'reports' && selectedReport && (
                <div className="space-y-6">
                  <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 flex justify-between items-center">
                    <div>
                      <p className="text-[#344767] font-bold text-sm mb-1">Generated Document Ready</p>
                      <p className="text-xs text-[#8392ab]">Please review the document before signing.</p>
                    </div>
                    <a href={selectedReport.fileUrl} download className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-[#344767] text-xs font-bold rounded-lg shadow transition">
                  ...
                  <div className="flex gap-4">
                    <button type="button" onClick={() => setSelectedReport(null)} className="flex-1 py-3 bg-gray-200 hover:bg-gray-100 text-[#344767] font-semibold rounded-xl transition-all text-sm">
                      Close Review
                    </button>
                    <button onClick={handleApproveReport} disabled={approving || !signature} className="flex-1 bg-gradient-to-br from-[#14172B] to-[#3A416F] hover:opacity-90 disabled:bg-gray-50 disabled:text-[#8392ab] text-white font-bold py-3 px-4 rounded-xl shadow-lg transition-all text-sm flex items-center justify-center gap-2">
